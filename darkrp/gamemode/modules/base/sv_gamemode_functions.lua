...
function GM:PlayerDeath(ply, weapon, killer)
    ...
    if GAMEMODE.Config.dropmoneyondeath then
        ...
        if money > 0 then
            ply:addMoney(-money)
            DarkRP.createMoneyBag(ply:GetPos(), money)
        end
    end
    ...
    DarkRP.log(ply:Nick() .. " a été tué par " .. KillerName .. " avec un(e) " .. WeaponName, Color(255, 190, 0))
end
...
local function ClearDecals()
    if GAMEMODE.Config.decalcleaner then
		...
		DarkRP.notifyAll(2, 4, "Les traces ont été supprimées")
    end
end
