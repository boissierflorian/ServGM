...
local function startSpectating(ply, target)
    ...
    local targetText = IsValid(target) and target:IsPlayer() and (target:Nick() .. " (" .. target:SteamID() .. ")") or IsValid(target) and "un objet" or ""
    ply:ChatPrint("Vous observez " .. targetText)
end

local function Spectate(ply, cmd, args)
    CAMI.PlayerHasAccess(ply, "FSpectate", function(b, _)
        if not b then ply:ChatPrint("Privilèges insuffisants !") return end
