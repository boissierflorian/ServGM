...
hook.Add("Initialize", "FSpectate", function()
    ...
    FAdmin.StartHooks["zzSpectate"] = function()
        ...
        -- Right click option
        FAdmin.ScoreBoard.Main.AddPlayerRightClick("Observer", function(ply)
        ...
        FAdmin.ScoreBoard.Player:AddActionButton("Observer", "fadmin/icons/spectate", Color(0, 200, 0, 255), function(ply) return canSpectate and ply ~= LocalPlayer() end, function(ply)
            if not IsValid(ply) then return end
            RunConsoleCommand("FSpectate", ply:UserID())
        end)
    end
...
local function drawHelp()
    ...
    draw.WordBox(2, 10, scrHalfH, "Clic gauche: (Dé)-sélectionner un joueur", "UiBold", uiBackground, uiForeground)
    draw.WordBox(2, 10, scrHalfH + 20, isRoaming and "Clic droit: avancer légèrement vers l'avant" or "Clic droit: activer la 3ème personne", "UiBold", uiBackground, uiForeground)
    draw.WordBox(2, 10, scrHalfH + 40, "Sauter: Arrêter d'observer", "UiBold", uiBackground, uiForeground)
    draw.WordBox(2, 10, scrHalfH + 60, "Recharger: Arrêter d'observer et se téléporter", "UiBold", uiBackground, uiForeground)

    if FAdmin then
        draw.WordBox(2, 10, scrHalfH + 80, "Ouverture du menu FAdmin", "UiBold", uiBackground, uiForeground)
        draw.WordBox(2, 10, scrHalfH + 100, "\touvrira leur(s) page(s)!", "UiBold", uiBackground, uiForeground)
    end
    ...
    for i = 1, #pls do
        ...
        draw.WordBox(2, x, y - 46, "Vie: " .. ply:Health(), "UiBold", uiBackground, uiForeground)
