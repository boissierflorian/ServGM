...
usermessage.Hook("FAdmin_kick_start", function()
    hook.Add("HUDPaint", "FAdmin_kick", function()
        ...
        draw.DrawNonParsedText("Vous allez être kické\nRaison: " .. KickText .. "\nQuitter de manière volontaire est aussi une option.", "HUDNumber5", ScrW() / 2, ScrH() / 2, Color(255, 0, 0, 255), TEXT_ALIGN_CENTER)
    end)
end)

usermessage.Hook("FAdmin_kick_cancel", function()
    hook.Remove("HUDPaint", "FAdmin_kick")
    FAdmin.Messages.AddMessage(4, "Le kick a été annulé")
end)
...
local BanText = "Raison"
local BanTimeText = "permanent"

usermessage.Hook("FAdmin_ban_start", function()
    hook.Add("HUDPaint", "FAdmin_ban", function()
        draw.RoundedBox(0,0,0, ScrW(), ScrH(), Color(0,0,0,255))
        draw.DrawNonParsedText("Vous allez être banni\nRaison: " .. BanText .. "\nDurée: " .. " " .. BanTimeText .. "\nQuitter le serveur n'annulera en aucun cas le ban.", "HUDNumber5", ScrW() / 2, ScrH() / 2, Color(0, 0, 255, 255), TEXT_ALIGN_CENTER)
    end)
end)

usermessage.Hook("FAdmin_ban_cancel", function()
    hook.Remove("HUDPaint", "FAdmin_ban")
    FAdmin.Messages.AddMessage(4, "Le ban a été annulé")
end)
...
local function showBanWindow(SteamID, NICK, time, reason)
    ...
    HourLabel:SetText("Heuress:")
    DayLabel:SetText("Jours:")
    WeekLabel:SetText("Semaines:")
    YearLabel:SetText("Années:")
...
FAdmin.StartHooks["CL_KickBan"] = function()
    ...
    FAdmin.ScoreBoard.Main.AddPlayerRightClick("Kick rapide", function(ply, Panel)
    ...
    FAdmin.ScoreBoard.Player:AddActionButton("Kick", "fadmin/icons/kick", nil, function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Kick", ply) end, function(ply)
        ...
        local Window = vgui.Create("DFrame")
        Window:SetTitle("Raison du kick")
        Window:SetDraggable( false )
        Window:ShowCloseButton( false )
        Window:SetBackgroundBlur( true )
        Window:SetDrawOnTop( true )
        ...
        local Text = vgui.Create("DLabel", InnerPanel)
            Text:SetText(NICK .. " sait qu'il va être kick\nPrenez tout votre temps pour écrire la raison.")
            Text:SizeToContents()
            Text:SetContentAlignment( 5 )
        ...
        local TextEntry = vgui.Create("DTextEntry", InnerPanel )
            ...
            TextEntry:SetText("Tapez la raison ici")
    ...
    FAdmin.ScoreBoard.Server:AddPlayerAction("Unban", function() return "fadmin/icons/ban", "fadmin/icons/disable" end, Color(0, 155, 0, 255), true, function(button)
        ...
        Frame:SetTitle("Gestion des débannissements")
        ...
        BanList:AddColumn("Pseudo")
        BanList:AddColumn("Durée")
        BanList:AddColumn("Raison")
        BanList:AddColumn("Auteur")
        BanList:AddColumn("Auteur SteamID")
