...
local function Kick(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Kick", target) then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
        if IsValid(target) then
            ...
            elseif stage == "update" then -- Update reason text
                ...
                Reason = Reason and string.gsub(Reason, ";", " ") or "Aucune raison n'a été donnée"

                game.ConsoleCommand(string.format("kickid %s %s\n", target:UserID(), "Kické par " .. name ..
                    " (" .. Reason .. ")"))
                ply.FAdminKickReason = nil
            end
        end
...
local function RequestBans(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "UnBan") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    ...
    for _, target in pairs(targets) do
        ...
        elseif stage == "update" then -- Update reason text
            ...
            if type(target) ~= "string" and IsValid(target) then
                ...
                target:Kick("banni par " .. nick .. " pour " .. TimeText .. " (" .. Reason .. ")")
            else
                for k,v in pairs(StartBannedUsers) do
                    if v == args[1] then
                        table.remove(StartBannedUsers, k)
...
local function canJoin(steamID)
    ...
    return false, "banni par " .. (ban.adminname or "<Inconnu>") .. "\nDurée: " .. timeText .. "\nRaison: " .. (ban.reason or "sans raison particulière")
end
