function FAdmin.PlayerActions.ConvertBanTime(time)
    ...
    elseif time >= 60 and time < 1440 then
        ...
        return math.floor(time / 60) .. " heure(s)" .. Add
    elseif time >= 1440 and time < 10080 then
        ...
        return math.floor(time / 1440) .. " jour(s)" .. Add
    elseif time >= 10080 and time < 525948 then
        ...
        return math.floor(time / 10080) .. " semaine(s)" .. Add
    elseif time >= 525948 then
        ...
        return math.floor(time / 525948) .. " année(s)" .. Add
    end
...
FAdmin.StartHooks["kickbanning"] = function()
    FAdmin.Messages.RegisterNotification{
        name = "kick",
        hasTarget = false,
        message = {"instigator", " a kické ", "extraInfo.1", " (", "extraInfo.2", ")"},
        receivers = "everyone",
    ...
    FAdmin.Messages.RegisterNotification{
        name = "ban",
        hasTarget = false,
        message = {"instigator", " a banni ", "extraInfo.1", " pour ", "extraInfo.2", " (", "extraInfo.3", ")"},
        receivers = "everyone",
        writeExtraInfo = function(info)
            net.WriteUInt(#info[1], 8)
            -- Manually send targets, because they might be gone from the client when kicked
    ...
    FAdmin.Messages.RegisterNotification{
        name = "unban",
        hasTarget = false,
        message = {"instigator", " a débanni ", "extraInfo.1", " (", "extraInfo.2", ")"},
        receivers = "everyone",
        writeExtraInfo = function(info)
