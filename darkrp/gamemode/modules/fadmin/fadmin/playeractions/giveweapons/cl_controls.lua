...
function PANEL:CreateAdminIcon()
    ...
    self.imgAdmin:SetTooltip("#Admins")
end
...
function PANEL2:BuildList()
    ...
    if not self.HideAmmo then
        ...
        AmmoCat:SetLabel("Donner des munitions")
    ...
    for k, weapon in pairs(Weapons) do
        ...
        Weapons[k] = weapon
        weapon.Category = weapon.Category or "Autres"
    ...
    for CategoryName, v in SortedPairs(Categorised) do
        ...
        for k, WeaponTable in SortedPairsByMemberValue(v, "PrintName") do
            ...
            local Tooltip =  Format("Nom: %s", WeaponTable.PrintName)
            if WeaponTable.Author ~= "" then Tooltip = Format("%s\nAuteur: %s", Tooltip, WeaponTable.Author) end
