local function GiveWeapon(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "giveweapon") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    ...
    FAdmin.Messages.ActionMessage(ply, targets, "Vous avez donné à %s un(e) " .. weapon, "%s vous a donné un(e) " .. weapon, "Don de %s d'un(e) " .. weapon)
