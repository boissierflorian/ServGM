...
FAdmin.StartHooks["Jailing"] = function()
    FAdmin.Messages.RegisterNotification{
        name = "jail",
        hasTarget = true,
        message = {"instigator", " a emprisonné ", "targets", " ", "extraInfo.1"},
        receivers = "involved+admins",
        writeExtraInfo = function(info) net.WriteUInt(info[1], 16) end,
        readExtraInfo = function()
            local time = net.ReadUInt(16)
            return {time == 0 and FAdmin.PlayerActions.commonTimes[time] or string.format("pour %s", FAdmin.PlayerActions.commonTimes[time] or (time .. " seconds"))}
        end
    }
    ...
    FAdmin.Messages.RegisterNotification{
        name = "unjail",
        hasTarget = true,
        message = {"instigator", " a libéré ", "targets"},
        receivers = "involved+admins",
    }
end
