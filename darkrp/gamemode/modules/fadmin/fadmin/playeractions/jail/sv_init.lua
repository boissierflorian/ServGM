local function Jail(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Jail", target) then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
        ...
        if not target:FAdmin_GetGlobal("fadmin_jailed") and JailType ~= "unjail" and string.lower(cmd) ~= "unjail"  then
            ...
            time = "pour " .. JailTime .. " secondes"
            if JailTime == 0 then time = "indéfiniment" end
        end
    end
