...
local function Cloak(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Cloak", target) then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    ...
    FAdmin.Messages.ActionMessage(ply, targets, "Vous avez rendu invisible %s", "Vous avez été rendu invisible par %s", "Dissimulation par %s")
...
local function UnCloak(ply, cmd, args)
    ...
    FAdmin.Messages.ActionMessage(ply, targets, "%s est de nouveau visible", "Vous êtes de nouveau visible", "Visibilité rétablie par %s")
