FAdmin.StartHooks["zz_Cloak"] = function()
    ...
    FAdmin.ScoreBoard.Player:AddActionButton(function(ply)
        if ply:FAdmin_GetGlobal("FAdmin_cloaked") then return "Rendre visible" end
        return "Rendre invisible"
    end, function(ply)
    ...
    function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Cloak", ply) end, function(ply, button)
        ...
        if not ply:FAdmin_GetGlobal("FAdmin_cloaked") then button:SetImage2("fadmin/icons/disable") button:SetText("Rendre visible") button:GetParent():InvalidateLayout() return end
        button:SetImage2("null")
        button:SetText("Rendre invisible")
