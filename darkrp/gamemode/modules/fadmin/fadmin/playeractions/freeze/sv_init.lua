local function Freeze(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Freeze", target) then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
...
FAdmin.StartHooks["Freeze"] = function()
    FAdmin.Messages.RegisterNotification{
        ...
        message = {"instigator", " a immobilisé ", "targets", " ", "extraInfo.1"},
    }
    ...
    FAdmin.Messages.RegisterNotification{
        ...
        hasTarget = true,
        receivers = "involved+admins",
        message = {"instigator", " a rétablit ", "targets"},
    }
