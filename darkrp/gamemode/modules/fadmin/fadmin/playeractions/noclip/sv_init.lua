...
local function SetNoclip(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "SetNoclip") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
        ...
        if IsValid(target) then
            if Toggle then
                ...
                if not FAdmin.Access.PlayerHasPrivilege(target, "Noclip") then
                    FAdmin.Messages.SendMessage(ply, 3, "Ceci n'est pas permanent ! Pour le rendre permanent il faut le définir avec un groupe personnalisé !")
                end
            else
...
FAdmin.StartHooks["zz_Noclip"] = function()
    FAdmin.Messages.RegisterNotification{
        ...
        hasTarget = true,
        receivers = "involved",
        message = {"instigator", " a activé le noclip pour ", "targets"},
    }
    ...
    FAdmin.Messages.RegisterNotification{
        ...
        hasTarget = true,
        receivers = "involved",
        message = {"instigator", " a désactivé le noclip pour ", "targets"},
    }
...
local function sendNoclipMessage(ply)
    ...
    FAdmin.Messages.SendMessage(ply, 4, "Noclip activé")
...
hook.Add("PlayerNoClip", "FAdmin_noclip", function(ply)
    if ply:FAdmin_GetGlobal("FADmin_DisableNoclip") then
        FAdmin.Messages.SendMessage(ply, 5, "Noclip désactivé !")
        return false
    end
