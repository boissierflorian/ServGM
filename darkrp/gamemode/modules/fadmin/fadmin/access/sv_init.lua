...
function FAdmin.Access.SetRoot(ply, cmd, args) -- FAdmin setroot player. Sets the player to superadmin
    if not FAdmin.Access.PlayerHasPrivilege(ply, "SetAccess") then
        FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !")
        FAdmin.Messages.SendMessage(ply, 5, "Veuillez entrer le RCon pour vous définir en tant que superadmin si vous êtes le propriétaire du serveur")
        return false
    end
    ...
    if (not plyGroup or group.immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à affecter un groupe qui est supérieur au votre à un joueur")
        FAdmin.Messages.SendMessage(ply, 5, "Veuillez entrer le RCon pour vous définir en tant que superadmin si vous êtes le propriétaire du serveur")
        return false
    end
    ...
    for _, target in pairs(targets) do
        ...
        FAdmin.Messages.SendMessage(ply, 2, "Joueur défini en tant que superadmin !")
    end
...
local function AddGroup(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "ManageGroups") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    local admin = tonumber(args[2])
    if not args[1] or not admin then FAdmin.Messages.SendMessage(ply, 5, "Arguments invalides !") return false end
    local privs = {}
    ...
    if (not plyGroup or immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à créér des groupes avec des privilèges plus élevés que les vôtres")
        return false
    end

    FAdmin.Access.AddGroup(args[1], admin, privs, immunity) -- Add new group
    FAdmin.Messages.SendMessage(ply, 4, "Groupe créé")
...
local function AddPrivilege(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "ManagePrivileges") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end

    local group, priv = args[1], args[2]

    if not FAdmin.Access.Groups[group] or not FAdmin.Access.Privileges[priv] then
        FAdmin.Messages.SendMessage(ply, 5, "Arguments invalides")
        return false
    end

    -- The player cannot add privileges that they themselves do not have
    if not FAdmin.Access.PlayerHasPrivilege(ply, priv) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à affecter des privilèges que vous ne possèdez pas")
        return false
    end
    ...
    if (not plyGroup or FAdmin.Access.Groups[group].immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à gérer les privilèges d'un groupe supérieur au votre")
        return false
    end
...
local function RemovePrivilege(ply, cmd, args)
    ...
    FAdmin.Messages.SendMessage(ply, 4, "Privilège supprimé !")
...
function FAdmin.Access.SetAccess(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "SetAccess") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    ...
    if not args[2] or not group and not admin then
        FAdmin.Messages.SendMessage(ply, 1, "Groupe introuvable")
        return false
    elseif args[2] and not group and admin then
        ...
        if (not plyGroup or immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
            FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à affecter quelqu'un à un groupe supérieur au votre")
            return false
        end
    ...
    if not targets and (string.find(args[1], "STEAM_") and #args == 5 or args[1] == "BOT") then
        ...
        FAdmin.Messages.SendMessage(ply, 4, "Groupe mis à jour !")
...
local function toggleImmunity(ply, cmd, args)
    ...
    if not args[1] then FAdmin.Messages.SendMessage(ply, 5, "Argument invalide !") return false end
    RunConsoleCommand("_FAdmin_immunity", args[1])
    local OnOff = (tonumber(args[1]) == 1 and "on") or "off"
    FAdmin.Messages.ActionMessage(ply, player.GetAll(), ply:Nick() .. (OnOff == "on" and " a activé" or " a désactivé") .. " l'immunité administrateur", 
		"L'immunité administrateur a été " .. (OnOff == "on" and "activée" or "désactivée"), 
		(OnOff == "on" and "Activation" or "Désactivation") .. " de l'immunité administrateur")
...
local function setImmunity(ply, cmd, args)
    ...
    -- Setting a group with a higher rank than one's own
    if (not plyGroup or FAdmin.Access.Groups[group].immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à modifier l'immunité de ce groupe")
        return false
    end

    if immunity > plyGroup.immunity and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à définir un niveau d'immunité plus grand que le votre")
        return false
    end
