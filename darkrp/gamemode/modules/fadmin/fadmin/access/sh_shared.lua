...
function FAdmin.Access.RemoveGroup(ply, cmd, args)
    ...
    if (not plyGroup or FAdmin.Access.Groups[args[1]].immunity > plyGroup.immunity) and not FAdmin.Access.PlayerIsHost(ply) then
        FAdmin.Messages.SendMessage(ply, 5, "Vous n'êtes pas autorisé à supprimer des groupes supérieurs au vôtre")
        return false
    end

    CAMI.UnregisterUsergroup(args[1], "FAdmin")

    FAdmin.Messages.SendMessage(ply, 4, "Groupe supprimé")
end
...
FAdmin.StartHooks["AccessFunctions"] = function()
    FAdmin.Messages.RegisterNotification{
        name = "setaccess",
        hasTarget = true,
        message = {"instigator", " a changé le groupe de ", "targets", " en ", "extraInfo.1"},
        receivers = "everyone",
