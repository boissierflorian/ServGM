...
FAdmin.StartHooks["Chatcommands"] = function()
    FAdmin.ScoreBoard.Server:AddServerSetting("Définir le préfixe des commandes FAdmin", "fadmin/icons/message", Color(0, 0, 155, 255), 
	function(ply) return FAdmin.Access.PlayerHasPrivilege(ply, "ServerSetting") end, function()
        local prefix = GetGlobalString("FAdmin_commandprefix")
        prefix = prefix ~= '' and prefix or '/'
        Derma_StringRequest("Définir le nouveau préfixe des commandes FAdmin", "Un caractère (au minimum) !", prefix, fp{RunConsoleCommand, "_Fadmin", "CommandPrefix"})
    end)
end
