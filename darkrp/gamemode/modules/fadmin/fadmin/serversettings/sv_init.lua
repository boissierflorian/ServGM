...
local function ServerSetting(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "ServerSetting") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    if not args[2] then FAdmin.Messages.SendMessage(ply, 5, "Argument incorrect") return false end
    ...
    FAdmin.Messages.ActionMessage(ply, player.GetAll(), args[1] .. " limité à " .. unpack(CommandArgs),
        args[1] .. " a été limité à" .. unpack(CommandArgs), args[1] .. " limité à " .. unpack(CommandArgs))
