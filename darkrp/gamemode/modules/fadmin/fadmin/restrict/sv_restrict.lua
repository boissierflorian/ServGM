...
local function DoRestrictWeapons(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "Restrict") then FAdmin.Messages.SendMessage(ply, 5, "Privilèges insuffisants !") return false end
    local Weapon = args[1]
    local Group = args[2]
    ...
    FAdmin.Messages.SendMessage(ply, 4, "L'arme est maintenant restreinte !")
...
local function UnRestrictWeapons(ply, cmd, args)
    ...
    FAdmin.Messages.SendMessage(ply, 4, "L'arme n'est plus restreinte !")
