...
function FAdmin.ScoreBoard.Player.Show(ply)
    ...
    for k, v in pairs(FAdmin.ScoreBoard.Player.Information) do
        ...
        if Value and Value ~= "" then
            ...
            Text:SetTooltip("Cliquer pour copier " .. v.name .. " dans le presse-papier")
            Text:SetMouseInputEnabled(true)

            function Text:OnMousePressed(mcode)
                self:SetTooltip(v.name .. " a été copié dans le presse-papier!")
                ChangeTooltip(self)
                SetClipboardText(Value)
                self:SetTooltip("Cliquer pour copier " .. v.name .. " dans le presse-papier")
            end
    ...
    FAdmin.ScoreBoard.Player.Controls.ButtonCat:SetLabel(" Options")
...
FAdmin.ScoreBoard.Player:AddInformation("Nom", function(ply) return ply:Nick() end)
FAdmin.ScoreBoard.Player:AddInformation("Meurtres", function(ply) return ply:Frags() end)
FAdmin.ScoreBoard.Player:AddInformation("Morts", function(ply) return ply:Deaths() end)
